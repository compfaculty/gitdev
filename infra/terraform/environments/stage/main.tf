# Staging stack — disjoint RFC1918 space so prod + stage can coexist in one Hetzner project.
locals {
  network_cidr     = "10.10.0.0/16"
  mgmt_subnet_cidr = "10.10.1.0/24"
  ci_subnet_cidr   = "10.10.2.0/24"
...
module "bastion" {
  ...
  cloud_init_user_data = file("${path.module}/templates/bastion-cloud-init.yaml.tpl")
}
...
module "ingress_edge" {
  ...
  labels               = local.common_labels
  cloud_init_user_data = file("${path.module}/templates/ingress-edge-cloud-init.yaml.tpl")
}
